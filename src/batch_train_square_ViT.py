...
if __name__ == "__main__":
    ...
    ## For Training Square patched
    Model_2D = True
    Model_Name = "TinyViT-v1"
    Image_Size = None
    depth = 64
    output_model_path='batch_trained_ViT_pose.pth'
    ...
    ## Resize data to be 256 x 256
    print('Resizing data')
    transform = torchvision.transforms.Resize((256,256))
    flattened_brain_data_all = transform(torch.from_numpy(flattened_brain_data_all)).numpy()

    print(len(flattened_brain_data_all))
    print(len(flattened_feature_data_all))
    print(len(discrete_data__all))
